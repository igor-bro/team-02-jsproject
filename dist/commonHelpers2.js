import{o as O}from"./assets/mob-menu-b8837aae.js";/* empty css                      */import{a as k,i as b}from"./assets/vendor-322b39ab.js";const A=document.querySelector(".loader");k.interceptors.request.use(e=>(A.style.display="inline-block",e),e=>console.error(e));k.interceptors.response.use(e=>(A.style.display="none",e),e=>Promise.reject(e));const T=document.querySelector(".back-to-top"),U=()=>{const e=Math.max(document.documentElement.scrollTop,document.body.scrollTop);e>0&&(window.requestAnimationFrame(U),window.scrollTo(0,e-e/8))};T.addEventListener("click",U);window.addEventListener("scroll",()=>{(window.pageYOffset||document.documentElement.scrollTop||document.body.scrollTop)>350?T.classList.add("active"):T.classList.remove("active")});var D=document.getElementById("myAudio");D.addEventListener("ended",function(){b.show({title:"Lets do it!",message:"Subscribe in the bottom, and learn about new exercises!",position:"bottomCenter",timeout:1e4,backgroundColor:"var( --dark-gray-hover)",messageColor:"var( --white-smoke)"})});const w=document.querySelector(".exercises-gallery-label"),n=document.querySelector(".exercises-gallery-group"),p=document.querySelector(".exercises-gallery-filter"),g=document.querySelector(".page-buttons-container");let c=null;const h=document.createElement("span"),C=document.querySelector("#filtre-key"),d=document.querySelector(".search-tool"),I=document.querySelector(".icon-search"),N=new URL("/team02-jsproject/assets/sprite-30cecc96.svg#icon-arrow",self.location),G=new URL("/team02-jsproject/assets/sprite-30cecc96.svg#icon-Star-1",self.location),z=new URL("/team02-jsproject/assets/sprite-30cecc96.svg#icon-lighticon",self.location),B=document.querySelector(".error-card-message"),Q="https://energyflow.b.goit.study/api/";let v=1;const l={content:null,title:null},V="https://energyflow.b.goit.study/api/filters";let a={filter:"Muscles",page:1,limit:8},m=0,f="filter";const _="muscles",Y=k.create({baseURL:V});p.addEventListener("click",function(e){if(H(),L(),m=0,d.style.display="none",e.target.tagName==="BUTTON"){f="filter";const t=e.target.getAttribute("name");S(),P(t)}});n.addEventListener("click",e=>{e.preventDefault();const t=e.target.closest(".exercises-item-background");if(t){f="workout",d.style.display="flex";const r=t.querySelector(".name-card").textContent;let s=t.querySelector(".type-card").textContent.toLowerCase().replace(/\s/g,"");if(s==="bodyparts"&&(s=s.slice(0,-1)),S(),se(r,s),m===0)return;m>=3?y(3):y(m)}});g.addEventListener("click",function(e){const t=e.target.closest(".page-button");if(t){const r=t.id;X(r)}});function P(e){d.style.display="none";let t={};switch(Z(),e){case"body":a.filter="Body parts",t=p.querySelectorAll(`button[name="${e}"]`);break;case"equipment":a.filter="Equipment",t=p.querySelectorAll(`button[name="${e}"]`);break;default:a.filter="Muscles",t=p.querySelectorAll(`button[name="${e}"]`)}J(t);const s=`?${new URLSearchParams(a).toString()}`;W(s).then(o=>j(o)).then(o=>y(o.totalPages,1)).catch(o=>console.error(o))}function J(e){const t=p.querySelectorAll("button"),r=Array.from(e);for(let s=0;s<t.length;s++){const o=t[s];r.includes(o)?(o.style.backgroundColor="var(--dark-gray)",o.style.color="var(--white)"):(o.style.backgroundColor="var(--white-smoke)",o.style.color="var(--black)")}}function K(){const t=`?${new URLSearchParams(a).toString()}`;W(t).then(r=>j(r)).catch(r=>console.error(r))}async function W(e){try{return(await Y.get(e)).data}catch(t){console.error(t)}}async function $(e){try{const t=await k.get(`${Q}exercises`,{params:e}),{totalPages:r,results:s}=t.data;if(s.length==0){n.innerHTML="",re();return}let o=s.reduce((E,i)=>{n.innerHTML="",g.innerHTML="";const u=te(i.rating),F=Number(i.rating).toFixed(1);return E+`<li class="gallery-item-list">
          <button type="button" class="gallery-button-item" data-exercise-id=${i._id}>
              ...
              <div class="workout-type-item">
                ...
                <p class="workout-name-item">${i.name}</p>
              </div>
          ...
          </li>`},"");m=t.data.totalPages,n.innerHTML="",n.insertAdjacentHTML("beforeend",o),n.addEventListener("click",async E=>{let i;const u=E.target;u&&u.closest(".gallery-button-item")&&(i=u.closest(".gallery-button-item").getAttribute("data-exercise-id"),await O(i))}),f="workout",r<3?y(r,e.page):y(3,e.page)}catch(t){console.log(t)}}function j(e){n.innerHTML="";const t=e.results.map(r=>`<li class="exercises-item-background">
      <a href="${r.imgUrl}">
      <div>
        <img
      ...
      <button class="page-button" type="button" id="${s}">${s}</button>`;g.insertAdjacentHTML("afterbegin",r)}function X(e){document.querySelectorAll(".page-button").forEach(r=>{if(r.id===e)switch(r.classList.add("active"),f){case"filter":a.page=e,K();break;case"workout":v=e;const s={limit:q(),page:v};s[l.content]=l.title,$(s);break}else r.classList.remove("active")})}function Z(){document.documentElement.clientWidth>=1440?(a.page=1,a.limit=12):(a.page=1,a.limit=8)}function ee(){L(),H(),n.innerHTML="",g.innerHTML=""}function te(e){let t="";e=Math.round(e);for(let r=0;r<e;r++)t+=`
